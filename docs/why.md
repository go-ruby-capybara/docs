...
`go-ruby-capybara/capybara` reimplements Ruby's `capybara` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-capybara/capybara` directly, with no Ruby runtime;
...
Because `capybara` is pure Go with cgo disabled, it also compiles to **WebAssembly**
